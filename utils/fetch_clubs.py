...
def main(out_folder):
    ...
    for row in table.find("tbody").find_all("tr"):
        ...
        if "vantar_logo" in img_url:
            ...
            path = os.path.join(out_folder, "%s%s" % (sanitized_name, ext))
...
if __name__ == "__main__":
    ...
    default_folder = os.path.join(BASE, "src", "images", "club-logos")
    parser.add_argument("--out_folder", default=default_folder)
    args = parser.parse_args()
    if not os.path.isabs(args.out_folder):
        raise RuntimeError("%s should be absolute path" % (args.out_folder,))
    if not os.path.isdir(os.path.dirname(args.out_folder)):
        raise RuntimeError("%s missing parent directory" % (args.out_folder,))
